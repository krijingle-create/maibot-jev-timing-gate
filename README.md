...
4. 机器人昵称不用填。插件通过官方 `config.get` 能力读取主程序的 `bot.nickname`
   （读不到时兜底读 `config/bot_config.toml`），用于「被 @ 时不抑制」的确定性豁免。
   只有自动读取失败、或你在群里换了别的群名片时，才需要在 `@豁免别名` 里手填。
5. 想先观察：打开影子模式，它只记录本该抑制的轮次，不改行为。
